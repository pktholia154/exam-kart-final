    ...
    return (
      <Link href={`/book/${book.seoslug}`} className={cn("flex gap-4 py-3 items-center active:scale-[0.98] transition-transform border-b border-gray-50 last:border-0", className)}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[0.5556rem] font-semibold text-[#8720BA] mb-0.5 uppercase tracking-wide truncate">{book.category}</p>
          <h3 className="text-xs font-bold text-gray-900 leading-tight mb-1 truncate">{book.title}</h3>
          {book.seoDescription && (
            <p className="text-[0.5556rem] text-gray-500 line-clamp-2 mb-1.5 leading-snug">{book.seoDescription}</p>
          )}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <Star className="w-3 h-3 fill-[#BA8720] text-[#BA8720]" />
              <span className="text-[0.5556rem] font-medium text-gray-700">{ratingText}</span>
            </div>
  ...
  return (
    <Link href={`/book/${book.seoslug}`} className={cn("flex flex-col gap-1 w-full active:scale-[0.98] transition-transform", className, layout === "rail" ? "w-[120px] shrink-0" : "")}>
      ...
      <div className="flex flex-col mt-0.5">
        <p className="text-[0.4444rem] font-extrabold text-[#8720BA] uppercase tracking-wider truncate mb-0.5">{book.category}</p>
        <h3 className="text-[0.5556rem] font-bold text-gray-900 leading-tight line-clamp-2 mb-0.5">{book.title}</h3>
        <div className="flex items-center justify-between mt-auto">
          <p className="text-[0.5556rem] font-black text-gray-900">₹{book.buyprice}</p>
          <div className="flex items-center gap-0.5">
            <Star className="w-2.5 h-2.5 fill-[#BA8720] text-[#BA8720]" />
            <span className="text-[0.4444rem] font-semibold text-gray-600">{ratingText}</span>
          </div>
        </div>
      </div>
    </Link>
